...
function AzureAuth() {
  ...
  const handleLogin = async () => {
    try {
      ...
      if (accounts.length === 0) {
        ...
        const jsonResponse = JSON.parse({ "data": "${username}" });
        postDataToBackend(jsonResponse);
  ...
  const postDataToBackend = async (loginData, projectData) => {
    try {
      var response1 = await fetch('http://localhost:8080/api/login', {
        method: 'POST',
        headers: {
      ...
      console.log('Project data posted to backend successfully');
    } catch (error) {
